...
- (void)setProjectID:(NSString *)pID {
  pID = [pID copy]; // if passed string is mutable, "copy" converts it to
                    // immutable string
  [projectID release];
  projectID = pID;
}
...
- (void)setInvoiceDate:(NSDate *)iDate {
  ...
  @try {
    ...
    NSString *modString =
        [[NSString alloc] initWithString:[dateFormatter stringFromDate:iDate]];
...
- (void)setMaterialsTotal:(double)mTotal {
  ...
  [self
      setSmaterialsTotal:[NSString stringWithFormat:@"%1.2f", materialsTotal]];
}
...
- (void)setInvoiceDeposit:(double)iDeposit {
  ...
  [self
      setSinvoiceDeposit:[NSString stringWithFormat:@"%1.2f", invoiceDeposit]];
}
...
- (void)encodeWithCoder:(NSCoder *)coder {
  ...
  [coder encodeObject:[NSString stringWithFormat:@"%1.2f", materialsTotal]
               forKey:@"materialsTotal"];
  [coder encodeObject:[NSString stringWithFormat:@"%1.2f", invoiceDeposit]
               forKey:@"invoiceDeposit"];
  [coder encodeObject:[NSString stringWithFormat:@"%1.2f", totalDue]
               forKey:@"totalDue"];
  [coder encodeObject:[NSString stringWithFormat:@"%1.2f", invoiceRate]
               forKey:@"invoiceRate"];
}
...
- (id)initWithCoder:(NSCoder *)coder {
  if (self = [super init]) {
    ...
    if ([coder decodeObjectForKey:@"materialsTotal"] != nil) {
      NSString *sM = [[NSString alloc]
          initWithString:[coder decodeObjectForKey:@"materialsTotal"]];
      sRate1 = [sM floatValue];
      [self setMaterialsTotal:sRate1];
    }

    if ([coder decodeObjectForKey:@"invoiceDeposit"] != nil) {
      NSString *sI = [[NSString alloc]
          initWithString:[coder decodeObjectForKey:@"invoiceDeposit"]];
    ...
    if ([coder decodeObjectForKey:@"totalDue"] != nil) {
      NSString *sT = [[NSString alloc]
          initWithString:[coder decodeObjectForKey:@"totalDue"]];
      sRate1 = [sT floatValue];
      [self setTotalDue:sRate1];
    }

    if ([coder decodeObjectForKey:@"invoiceRate"] != nil) {
      NSString *sH = [[NSString alloc]
          initWithString:[coder decodeObjectForKey:@"invoiceRate"]];
