...
- (void)evaluationCheck {
  ...
  NSString *serialNum = [[NSString alloc]
      initWithString:@"TC5597332243"]; // this is also set in
...
- (IBAction)serialNumberEntry:(id)sender {
  NSString *serialNum = [[NSString alloc]
      initWithString:@"TC5597332243"]; // valid registration number
  if ([[regNumberField stringValue] isEqual:serialNum]) {
    ...
    [regDict writeToFile:regPath
              atomically:nil]; // write back to rgLicTBBTS.plist file

    // int serNum = NSRunAlertPanel(NSLocalizedString(@"regCompleted",
    // @"regCompleted"), NSLocalizedString(@"regThanks", @"regThanks"),nil,
    // nil,nil,nil);
    ...
    // int serNum = NSRunAlertPanel(NSLocalizedString(@"regError", @"regError"),
    // NSLocalizedString(@"regErrorMessage", @"regErrorMessage"),nil,
    // nil,nil,nil);
  }
}
...
- (IBAction)deleteInvoice:(id)sender {
  ...
  // Localized example
  validateAction = NSRunAlertPanel(
      NSLocalizedString(@"delInvoice", @"delInvoice"),
      NSLocalizedString(@"delInvoiceConfirm", @"delInvoiceConfirm"),
      NSLocalizedString(@"sYes", @"sYes"), NSLocalizedString(@"sNo", @"sNo"),
  ...
  if (validateAction == 1) {
    ...
    if ([self getSelTabIndex] == 4) {
      ...
      if ([rows count] > 0) {
        ...
        NSBeep();
        err = NSRunAlertPanel(
            NSLocalizedString(@"selProject", @"selProject"),
            NSLocalizedString(@"selProjInvoice", @"selProjInvoice"), @"sOK",
            nil, nil);
      }
    }
  }
...
- (void)viewSelectedSessions {
  ...
  if (selRow >= 0) {
    ...
    [selSessions
        addObjectsFromArray:storedSessions]; // add all if nothing selected
    // add buttons to list view for notes
    [self newSessionButtons:selSessions:0];
  }
}
...
- (NSString *)createProjectID {
  ...
  // check stored sessions and see if project id ref is used already, if so,
  // generate a different project id;
  for (i = 0; i < [storedSessions count]; i++) {
  ...
  for (i = 0; i < [arrProjects count]; i++) {
    ...
    for (z = 0; z < [storedSessions count]; z++) {
      ...
      if ([tempID isEqual:pProjectID]) {
        ...
        tSecs =
            [[ttSession endTime] timeIntervalSinceDate:[ttSession startTime]];
...
- (void)startClockTimer:(id)sender {
  ...
  NSDateFormatter *dateFormatter =
      [[NSDateFormatter alloc] initWithDateFormat:@"%H:%M:%S"
                             allowNaturalLanguage:NO];
  ...
  [self newSessionButtons:
                 uSession:0]; // to reset buttons to unpressed state only.
...
- (IBAction)raiseInvoiceWindow:(id)sender {
  ...
  if ([self getSelTabIndex] == 4) {
    ...
    NSMutableArray *arrProfiles = [[NSMutableArray alloc]
    ...
    if ([arrProfiles count] == 0) {
      ...
      int validateAction;
      validateAction =
          NSRunAlertPanel(NSLocalizedString(@"profCheck", @"profCheck"),
                          NSLocalizedString(@"profCheckMsg", @"profCheckMsg"),
                          NSLocalizedString(@"0K", @"OK"), nil, nil);
      ...
      if ([[uProfile profileName] isEqual:emptystr]) {
        int validateAction;
        validateAction = NSRunAlertPanel(
            NSLocalizedString(@"profCheck", @"profCheck"),
            NSLocalizedString(@"profNameCheckMsg", @"profNameCheckMsg"),
            NSLocalizedString(@"0K", @"OK"), nil, nil);
      }
      ...
      for (i = 0; i < [arrClients count]; i++) {
        ...
        if ([x isEqual:y]) {
          [txtClientAddress
              setStringValue:[[arrClients objectAtIndex:i] streetAddress]];
          ...
          [txtClientZip
              setStringValue:[[arrClients objectAtIndex:i] postalCode]];
        }
      }
      ...
      if ([self getDateFormatSetting] == 1) {
        dateFormatter = [[NSDateFormatter alloc] initWithDateFormat:@"%d/%m/%y"
                                               allowNaturalLanguage:NO];
      } else {
      ...
      NSImage *invImage = [[NSImage alloc]
          initByReferencingFile:[settingsDict objectForKey:@"invoiceImage"]];
...
- (void)newSessionButtons:(id)tSession:(int)type {
  ...
  /*
          ...
          if (type==1){
                  ...
                  desiredColumn = [sessionsView
     tableColumnWithIdentifier:@"clientName"];
                  ...
                  [cellPU setTarget:self];
                  desiredColumn = [sessionsView
     tableColumnWithIdentifier:@"projectName"];
                  [desiredColumn  setDataCell:cellPU];
                  [cellPU release];
          }
  */
...
- (IBAction)promptQTchange:(id)sender {
  ...
  if ([self getSelTabIndex] == 2) {
    for (i = 0; i < [arrSessions count]; i++) {
      ...
      if (![[tSession clientName] isEqual:@""] &&
          ![[tSession projectName] isEqual:@""]) {

        if (![tSession projectIDref]) { // save if no project id exists

          // err = NSRunAlertPanel(@"Save Session", @"Would you like to save the
          // selected quick timer session/s?", @"Yes", @"No", @"Cancel");
          Project *nProject = [[Project alloc] init];
          // Client *nClient =[[Client alloc] init];

          // selected client and project names
          NSString *sClientName =
              [[NSString alloc] initWithString:[tSession clientName]];
          NSString *sProjectName =
              [[NSString alloc] initWithString:[tSession projectName]];

          // get project id for selected projectname
          for (z = 0; z < [arrProjects count]; z++) {
            nProject = [arrProjects objectAtIndex:z];
            if ([[nProject projectName] isEqual:sProjectName] &&
                [[nProject clientName] isEqual:sClientName]) {
              [tSession setProjectIDref:[nProject projectID]]; // set correct
                                                               // project id
        ...
        err = NSRunAlertPanel(NSLocalizedString(@"saveSess", @"saveSess"),
                              NSLocalizedString(@"saveSessMsg", @"saveSessMsg"),
                              NSLocalizedString(@"sOK", @"sOK"), nil, nil);
      }
    }
  }
}
...
- (IBAction)addSession:(id)sender {
  ...
  if (selRow >= 0) {
    ...
    err =
        NSRunAlertPanel(NSLocalizedString(@"selProject", @"selProject"),
                        NSLocalizedString(@"selProjSession", @"selProjSession"),
                        NSLocalizedString(@"sOK", @"sOK"), nil, nil);
    [selProject release];
  }
...
- (IBAction)addProject:(id)sender {
  ...
  if (selRow >= 0) {
    ...
    err = NSRunAlertPanel(NSLocalizedString(@"selClient", @"selClient"),
                          NSLocalizedString(@"selClientMsg", @"selClientMsg"),
                          NSLocalizedString(@"sOK", @"sOK"), nil, nil);
    [selClient release];
  }
  [nProject release];
...

{
  ...
  @try {
    ...
    NSDateFormatter *dateFormat =
        [[NSDateFormatter alloc] initWithDateFormat:formatString
                               allowNaturalLanguage:NO];
    [[dateCol dataCell] setFormatter:dateFormat];

    // NSLog(@"COLUMN FORMATTER ========  %@",[[[dateCol dataCell] formatter]
    // dateFormat] );
...
- (IBAction)deleteSession:(id)sender {
  ...
  validateAction = NSRunAlertPanel(
      NSLocalizedString(@"delSess", @"delSess"),
      NSLocalizedString(@"delSessConfirm", @"delSessConfirm"),
      NSLocalizedString(@"sYes", @"sYes"), NSLocalizedString(@"sNo", @"sNo"),
      NSLocalizedString(@"Cancel", @"Cancel"));
  ...
  if (validateAction == 1) {
    ...
    if ([self getSelTabIndex] == 2) {
      ...
      if ([rows count] > 0) {
        ...
        x = [[NSString alloc]
            initWithString:[[arrSessions objectAtIndex:row] sessionID]];
        for (i = 0; i < [storedSessions count]; i++) {

          @try {
            y = [[NSString alloc]
                initWithString:[[storedSessions objectAtIndex:i] sessionID]];
            if ([x isEqual:y]) {
...
- (IBAction)deleteProject:(id)sender {
  ...
  validateAction = NSRunAlertPanel(
      NSLocalizedString(@"delProj", @"delProj"),
      NSLocalizedString(@"delProjConfirm", @"delProjConfirm"),
      NSLocalizedString(@"sYes", @"sYes"), NSLocalizedString(@"sNo", @"sNo"),
      NSLocalizedString(@"Cancel", @"Cancel"));
...
- (IBAction)deleteClient:(id)sender {
  ...
  validateAction = NSRunAlertPanel(
      NSLocalizedString(@"delClient", @"delClient"),
      NSLocalizedString(@"delClientConfirm", @"delClientConfirm"),
      NSLocalizedString(@"sYes", @"sYes"), NSLocalizedString(@"sNo", @"sNo"),
      NSLocalizedString(@"Cancel", @"Cancel"));
...
- (id)tableView:(NSTableView *)atableView
    objectValueForTableColumn:(NSTableColumn *)tableViewColumn
                          row:(int)rowIndex {
  switch ([self getSelTabIndex]) {
  case 0: {
    NSString *identifier = [tableViewColumn
        identifier]; // identifier for the column(set in Interface builder)
    Client *nClient = [arrClients objectAtIndex:rowIndex];
    return [nClient
        valueForKey:identifier]; // value of the attribute named identifier
  }
...
- (void)tableView:(NSTableView *)atableView
  ...
  switch ([self getSelTabIndex]) {
  case 0: {
    NSString *identifier = [tableColumn
        identifier]; // identifier for the column(set in Interface builder)
    Client *nClient = [arrClients objectAtIndex:rowIndex];
    [nClient setValue:anObject
               forKey:identifier]; // value of the attribute named identifier
    break;
  }
...
- (NSString *)pathForDataFile:(int)sFile {
  ...
  NSString *folder =
      @"~/Library/Application Support/TrackAndBill_OSX"; // destination folder
...
- (IBAction)print:(id)sender {
  ...
  NSView *v = invoiceBox;
  printOp = [NSPrintOperation printOperationWithView:v printInfo:printInfo];
...
- (IBAction)exportProjectSessions:(id)sender {
  ...
  if (selRow >= 0) {
    ...
    headerString = [NSString
        stringWithFormat:
            @",'Client','Project','Date','Start Time','End Time','Notes',"];
    ...
    err = NSRunAlertPanel(NSLocalizedString(@"selProject", @"selProject"),
                          NSLocalizedString(@"selProjExport", @"selProjExport"),
                          @"sOK", nil, nil);
    [selProject release];
  }
}
...
- (void)dealloc {
  // int err;
  // err = NSRunAlertPanel(@"Select project", @"Please select a project for your
  // new session.", @"OK", nil, nil);
